...
자소서 AI의 목적은 사용자가 입력한 공고 분석 결과, 자기소개서, 이력서, 포트폴리오를 기준으로 자소서를 평가하고, 보완 방향과 수정 방향 예시를 제공하는 것이다.

이 서비스는 AI가 점수를 임의로 찍는 구조가 아니라, `JD 분석 -> 지원자 근거 추출 -> 항목별 평가 -> 수정 방향 예시 작성 -> 예시 재검토 -> 서버 검증` 순서로 동작한다.

자료/요구사항 기준상 자소서 AI는 독립적인 자기소개서 생성 기능이 아니라 피드백/평가 기능이다. 따라서 Draft 계열 Agent는 최종 제출본을 생성하는 기능이 아니라, 사용자가 개선 방향을 이해하기 위한 학습용 예시를 만드는 내부 노드로만 본다.
...
역할:
- 평가 결과와 사용자 원문을 바탕으로 수정 방향 예시를 생성한다.
- 문항 수를 원본과 맞춘다.
- 각 문항에 소제목을 붙인다.
...
- 수정 방향 예시는 사용자가 어떻게 고치면 되는지 방향을 보여주는 학습용 결과물이다.
...
왜 필요한가:
- 생성 AI는 수정 방향 예시를 그럴듯하게 만들 수 있지만, 입력 근거에 없는 내용을 섞을 수 있다.
- 그래서 수정 방향 예시를 사용자에게 보여주기 전에 한 번 더 검토한다.
...
- 수정 방향 예시 작성이 실패해도 피드백 리포트는 반환 가능하다.
...
질문: 수정 방향 예시는 믿을 수 있나?

답변: 수정 방향 예시도 바로 보여주는 것이 아니라 Draft Reviewer Agent가 문항 수, 형식, JD 연결, 과장 여부를 다시 검사한다. 다만 수정 방향 예시는 최종 제출본이 아니라 사용자가 수정 방향을 이해하기 위한 학습용 결과물이다.
...
4. `confidence`가 낮으면 수정 방향 예시 작성 전에 사용자에게 추가 자료 입력을 요청한다.

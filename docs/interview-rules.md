...
- 기본 질문은 LLM question planner가 JD/사용자 문서 기준으로 생성한다.
- 생성 질문은 LLM guardrail과 서버 하네스를 모두 통과해야 한다.
- 실패 시 서버 fallback 질문을 사용한다.
- JD_FIT 질문은 반드시 직무상세, 지원자격, 우대사항의 구체 키워드와 연결한다.
- `인턴십 역량`, `핵심 역량`처럼 공고 근거가 약한 추상 질문은 금지한다.
...
- 5문항 미만 종료는 `CANCELLED` 상태이며 리포트 없이 종료 처리
